...
- A route may extend beyond the commander's remaining movement. The portion traversable this month is yellow and the future portion is gray. Confirming a route visibly animates the commander through each traversed tile, stopping at the same discovery, encounter, or movement-limit tile determined by the gameplay rules. Reduced-motion preferences shorten this travel animation. Confirming a long route moves only across the yellow portion and spends the available movement; the destination can be plotted again after movement refreshes.
...
The Western Marches contain a sawmill that produces 10 timber per month, a quarry that produces 8 stone per month, and the Blackridge material works that produces 2 units of the current age's strategic material per month. Each producer occupies a multi-cell footprint with one passable interaction square immediately outside its artwork; route selection anywhere on the artwork resolves to that adjacent entrance, so the commander never occupies the structure itself. Neutral producers are visibly occupied by a bandit unit at that entrance. The first right-click reveals the bandits' approximate force, and that force must be defeated before the site transfers to the player and begins monthly production. After victory the commander remains beside the producer on its entrance square. Producers persist after capture and are not consumed like pickups. The Blackridge site and its output visibly modernize with the civilization's age.
...
- **Strategic material:** one persistent stockpile whose grounded identity changes with the current age: Bronze, Iron, Steel, Saltpeter, Oil, then Fuel. Map pickups, the Blackridge producer, resource labels, and artwork all use the current identity. The stockpile pays the added material cost of the Foundry, Siege Workshop, War College, Cavalry School, and Citadel, as well as each recruited Artillery, Armor, or Aircraft unit.
- **Research:** progress through the current age's technology tree.

Food is not a player-managed resource. The changing strategic-material identity remains one number rather than six simultaneous stockpiles, avoiding needless bookkeeping while keeping each age grounded.
...
- Adventure-map objects use recognizable artwork without permanent nameplates. Timber is shown as cut logs, stone as a dressed-stone cache, and gold as a pay chest. Strategic-material pickups change by age: bronze ingots, iron billets, steel bars, saltpeter barrels, oil drums, and modern fuel canisters. Their names and state remain available through hover text and accessible labels.
- Cities and all three producer types use distinct, purpose-made artwork for every age: Ancient, Classical, Medieval, Gunpowder, Industrial, and Modern. Ancient construction avoids Roman or Classical architecture; Greco-Roman forms belong only to the Classical set. Every city is presented as a complete landmark location on its own irregular clearing above roads and terrain. The clearing masks roads beneath the settlement so routes visually approach its illuminated gate instead of cutting across or washing out the city artwork.
